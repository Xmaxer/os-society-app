...
function useApi<DataType, VariablesType = {}>({query}: IUseApiProps) {
    ...
    const [error, setError] = useState<undefined | APIError<IGraphQLError>>(undefined);
    const [data, setData] = useState<DataType | undefined>(undefined);
    const [mutationCall] = useMutation<DataType, VariablesType, IGraphQLError>(print(query.query));
    const [queryCall] = useManualQuery<DataType, VariablesType, IGraphQLError>(print(query.query));
    const call = query.type === MUTATION_OPERATION ? mutationCall : queryCall;

    const request = ({variables = {} as VariablesType, handleComplete, handleSuccess, handleError}: IRequestHandler<DataType, VariablesType>) => {
        setLoading(true);
        call(variables ? {variables: {...variables}} : {}).then((response) => {
            if (!response.error) {
                ...
                if (response.error && response.error.fetchError && response.error.fetchError.message === "Failed to fetch") {
                    ...
                    return;
                } else if (response.error.graphQLErrors && response.error.graphQLErrors.find((e: any) => e.extensions && e.extensions.code === -1)) {
