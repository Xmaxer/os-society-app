...
import {OrderEnum, PlayerOrderEnum, Players, Players_players, PlayersVariables} from "../assets/api/apiInterfaces";
...
function PlayerTable({days, ranks, search}: IPlayerTableProps) {
    ...
    const [orderBy, setOrderBy] = useState<PLAYER_ORDER_ENUM | PlayerOrderEnum>(PlayerOrderEnum.CREATED_AT);
    const [page, setPage] = React.useState(0);
    const [rowsPerPage, setRowsPerPage] = React.useState(rowsPerPageOptions[0]);
    const [players, setPlayers] = useState<Array<Players_players>>([]);
    const [totalPlayers, setTotalPlayers] = useState(0);
    const {request, loading} = useApi<Players, PlayersVariables>({query: PLAYERS_QUERY});
    ...
    const handleSort = (id: PlayerOrderEnum | PLAYER_ORDER_ENUM) => {
        setOrder(orderBy === id && order === OrderEnum.ASC ? OrderEnum.DESC : OrderEnum.ASC);
        setPage(0)
        setOrderBy(id)
    };
