...
function ErrorSnackbar() {
    ...
    return (
        <>
            <Snackbar open={errors && !!errors.fetchError} onClose={onClose} autoHideDuration={5000}>
                <Alert severity={ERROR}
                       onClose={onClose}>{errors && errors.fetchError ? errors.fetchError.message : ""}</Alert>
            </Snackbar>
            <Snackbar open={errors && !!errors.graphQLErrors} onClose={onClose} autoHideDuration={5000}>
                <Alert severity={ERROR}
                       onClose={onClose}>{errors && errors.graphQLErrors ? errors.graphQLErrors.map((m) => m.message + "\n") : ""}</Alert>
            </Snackbar>
            <Snackbar open={errors && !!errors.httpError} onClose={onClose} autoHideDuration={5000}>
                <Alert severity={ERROR}
