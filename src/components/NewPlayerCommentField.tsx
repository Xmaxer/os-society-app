...
function NewPlayerCommentField({changeHandler, value, ...rest}: INewPlayerCommentFieldProps) {
    const classes = useStyles();
    return (
        <StyledTextField name={"comment"}
                         multiline={true} inputProps={{maxLength: 200}}
                         onChange={changeHandler} value={value} label={'comment'}
                         className={classes.formCommentField} rowsMax={1} {...rest}/>
    );
}
