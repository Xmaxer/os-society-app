...
function InformationSnackbar() {
    ...
    return (
        <Snackbar open={info && !!info.message} onClose={onClose} autoHideDuration={5000}>
            <Alert severity={info && info.type ? info.type : SUCCESS}
                   onClose={onClose}>{info && info.message ? info.message : ""}</Alert>
        </Snackbar>
    );
}
