...
function NewPlayerFormDialog({successHandler, open, cancelHandler}: INewPlayerFormDialogProps) {
                ...
                {({values, handleChange, handleSubmit, setFieldValue, isValid}) => (
                    ...
                    <form className={classes.formContainer} onSubmit={(event) => {
                        ...
                        <div className={classes.fieldContainer}>
                            <NewPlayerUsernameTextField changeHandler={handleChange} value={values.username}/>
                            <NewPlayerJoinDateField changeHandler={(date) => {
                                setFieldValue('joinDate', date ? toDate(date) : null)
                            }} value={values.joinDate}/>
                            <NewPlayerRankField changeHandler={(event, option) => {
                                setFieldValue('rank', option ? option.id : null)
                            }}/>
                            <NewPlayerPreviousNameField changeHandler={(event, option) => {
                                setFieldValue('previousName', option)
                            }} value={values.previousNames} fieldHandler={setFieldValue}/>
                            <NewPlayerCommentField changeHandler={handleChange} value={values.comment}/>
                        </div>
                        <div className={classes.actionContainer}>
                            <StyledButton variant={'contained'}
                                          onClick={(event) => handleSubmit(event as unknown as React.FormEvent<HTMLFormElement>)}
                                          disabled={!isValid}>Save</StyledButton>
                            <StyledButton variant={'contained'}
