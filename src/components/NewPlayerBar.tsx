...
function NewPlayerBar({handleAddNewPlayer}: INewPlayerBarProps) {
    ...
    return (
        <div className={classes.addNewHeader}>
            {edit ? !matches ? <NewPlayerForm successHandler={handleSuccess} cancelHandler={handleCancel}/> :
                <NewPlayerFormDialog successHandler={handleSuccess} open={edit && matches}
                                     cancelHandler={handleCancel}/>
                : <StyledButton onClick={handleNewPlayerClick} variant={'outlined'}>Click here to add a new
                    player</StyledButton>}
        </div>
    );
}
