...
function PlayerTableRow({defaultPlayer, onDelete}: PlayerTableRowProps) {
    ...
    const handleChange = (name: string, value?: string | null | Array<string> | Date | number) => {
        handlePlayerCall({variables: {...player, [name]: value}, handleSuccess: onEditSuccess})
    }
    ...
    return (
        <TableRow key={player.id}>
            ...
            <EditableDatePickerCell width={'10%'} defaultValue={player.joinDate}
                                    id={player.id} name={'joinDate'} onChange={handleChange} align={'left'}/>
